...
sc_meta=CSV.File("IRI_metadata.txt.tar.gz") |> DataFrame
rename!(sc_meta,append!(["Column1"], names(sc_meta))[1:end-1])
...
rename!(sc_count,append!(["Column1"], names(sc_count))[1:end-1])
